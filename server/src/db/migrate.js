...
function runMigrations() {
  ...
  try {
    ...
    // Check if activity_log table exists and needs migration
    try {
      const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='activity_log'").get();
      if (!tables) {
        console.log('✓ activity_log table will be created by initSchema');
      } else {
        ...
        if (!hasUserId) {
          ...
          for (const log of existingLogs) {
            ...
            db.prepare(`
              INSERT INTO activity_log (id, user_id, user_email, action, detail, created_at)
              VALUES (?, ?, ?, ?, ?, ?)
            `).run(log.id, user?.id || null, log.user_email, log.action, log.detail, log.created_at);
          }
